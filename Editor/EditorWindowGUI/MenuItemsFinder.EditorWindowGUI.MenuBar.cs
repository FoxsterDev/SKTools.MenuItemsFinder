    ...
    internal partial class MenuItemsFinder
    {
        ...
        private void DrawMenuBar()
        {
            ...
            if (_prefs.ShowMenuBar)
            {
                if (_menuBar == null)
                {
                    _menuBar = new GuiLayoutHorizontal(new GuiListElements(
                        GuiElementsFactory.CreateLayoutToggleWithFixedWidth("Hide Unity Items",
                            (v)=> { _prefs.HideUnityItems = v; }, () => _prefs.HideUnityItems, 120),
                        GuiElementsFactory.CreateLayoutToggleWithFixedWidth("Show Only Starred",
                            (v)=> { _prefs.ShowOnlyStarred = v; }, () => _prefs.ShowOnlyStarred, 120),
                        GuiElementsFactory.CreateLayoutToggleWithFixedWidth("Hide Missed Items",
